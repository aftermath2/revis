...
import { ConfigKey, WalletType } from '../types';

export interface Backend {
    init(): Promise<void>;
    disconnect(): void;
    isAvailable(): boolean;
    getInvoice(zapRequest: EventTemplate, amount: number, lud16?: string): Promise<string>;
...
class Wallet {
    ...
    async getInvoice(zapRequest: EventTemplate, amount: number, lud16?: string): Promise<string> {
        return await this.backend.getInvoice(zapRequest, amount, lud16);
    }
